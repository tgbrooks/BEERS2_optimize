...
# Load performance scores of all the previous round
scores = {}
...
rng = np.random.default_rng([snakemake.wildcards.batch, snakemake.wildcards.num])
initialize_from = rng.choice(best_scorers)
with open(snakemake.input.configs[initialize_from]) as base_config_file:
...
def perturb_num(name, std, mode="linear", min_val=None, max_val=None):
    if mode == "linear":
        config[name] = rng.normal(config[name], std)
    elif mode == "log":
        config[name] = np.exp(rng.normal(np.log(config[name]), std))
...
with open(snakemake.output.config, "wt") as out:
